...
class Userdata extends AdminBase
{
    ...
    public function index($id = NULL)
    {
        ...
        $this->data['item'] = $this->base_model->get_join_item('result', 'users.*', NULL, 'users', ['users_groups'], ['users.id=users_groups.user_id'], ['inner'], ['group_id' => 2]);
    ...
    {
        $sql = "SELECT users.`username`, users.`first_name`, users_resellers.reseller_id, users_resellers.user_id
                FROM users, `users_resellers`, `users_groups`, groups
                WHERE users.`id` = `users_groups`.`user_id`
                AND `groups`.`id` = `users_groups`.`group_id`
                AND users_resellers.`user_id` = users.`id`
                AND users_resellers.`reseller_id` = $id
                AND users.`active` = 0";
        $this->data['detail'] = $this->db->query($sql)->result_array();
        $this->adminview('admin/userdata/detail', $this->data);
    }
    ...
    public function activation($id_reseller)
    {
        $update = "UPDATE users, `users_groups`, groups, users_resellers
                SET users.`active`= 1 
                WHERE users.`id`= `users_groups`.`user_id`
                AND groups.`id` = users_groups.`group_id`
                AND groups.`id` = 2
                AND users_resellers.`user_id` = users.`id`
                AND users_resellers.reseller_id = $id_reseller";
        $this->db->query($update);
        $this->session->set_flashdata('msg', 'Akun berhasil diaktifkan');
        redirect('manage/userdata');
    }
    public function user_active($id_reseller, $id)
    {
        $sql = "UPDATE users, `users_groups`, groups, users_resellers
                SET users.`active`= 1 
                WHERE users.`id`= `users_groups`.`user_id`
                AND groups.`id` = users_groups.`group_id`
                AND groups.`id` = 2
        ...
        $this->session->set_flashdata('msg', 'Akun berhasil diaktifkan');
        redirect("manage/userdata/detail_school/$id_reseller");
    }
    // public function nonactive($id)
    // {
    //     $update = "UPDATE users SET users.`active`= 0 WHERE users.`id`=$id";
    //     $this->db->query($update);
    //     $this->session->set_flashdata('msg', 'Akun berhasil di non aktifkan');
    //     redirect('manage/userdata');
    // }

    public function delete_all($reseller_id)
    {
        $delete_all = "DELETE users.* FROM users, `users_groups`, groups, users_resellers
                WHERE users.`id`= `users_groups`.`user_id`
                AND groups.`id` = users_groups.`group_id`
                AND groups.`id` = 2
                AND users_resellers.`user_id` = users.`id`
                AND users_resellers.reseller_id = $reseller_id
                AND users.`active` = 0";
        $this->db->query($delete_all);
        $this->session->set_flashdata('msg', 'Akun berhasil dihapus');
        redirect('manage/userdata');
    }

    public function user_delete($reseller_id, $id)
    {
        $delete_user = "DELETE users.* FROM users, `users_groups`, groups, users_resellers
                        WHERE users.`id`= `users_groups`.`user_id`
                        AND groups.`id` = users_groups.`group_id`
                        AND groups.`id` = 2
                        AND users.`id` = $id
        ...
        $this->session->set_flashdata('msg', 'Akun berhasil dihapus');
        redirect("manage/userdata/detail_school/$reseller_id");
    }
}
