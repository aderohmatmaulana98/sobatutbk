...
class Auth extends CI_Controller
{
	...
	public function login()
	{
		...
		if ($this->form_validation->run() === TRUE) {
			...
			if ($this->ion_auth->login($this->input->post('identitylog'), $this->input->post('passwordlog'), $remember)) {
				...
				if ($this->base_model->get_join_item('row', '*', NULL, 'users', ['users_groups'], ['users.id=users_groups.user_id'], ['inner'], ['users.id' => $this->session->userdata('user_id'), 'group_id' => 4])) {
	...
	public function edit_user($id)
	{
		...
		} elseif($this->base_model->get_item('row', 'users_groups', '*', ['user_id' => $id, 'group_id' => 4]))  {
			$this->load->view('school/template/header', $this->data);
