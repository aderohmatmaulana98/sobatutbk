...
class Base_model extends CI_Model
{
    ...
    {
        $data_tipe = "SELECT kategori_soal.category
        FROM `kategori_soal`
        GROUP BY `kategori_soal`.`category`";
        return $this->db->query($data_tipe)->result_array();
    }

    public function getdatajenisujian($tipe)
    {
        //         $data_mapel = "SELECT `kategori_soal`.`subject`
        // FROM kategori_soal
        // WHERE `kategori_soal`.`tipe` = 'TKA'"
        return $this->db->get_where('kategori_soal', ['tipe' => $tipe])->result();
    }
}
